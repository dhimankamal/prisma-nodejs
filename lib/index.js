...
const getPostData = async page => {
  try {
    const res = await axios({
      method: 'GET',
      url: `https://themoviesverse.org.in/wp-json/wp/v2/posts?per_page=20&page=${page}`
    })
...
const updatePostData = async () => {
  try {
    ...
    //for (let index = 1; index < 10; index++) {
      ...
      let index = 1
      const data = postdata   //await getPostData(index)
      if (data.length) {
...
const getCategory = async (page) => {
  try {
    const res = await axios({
      method: 'GET',
      url: `https://themoviesverse.org.in/wp-json/wp/v2/categories?per_page=20&page=${page}`
    })
